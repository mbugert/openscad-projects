...
// TODO mbugert: added 2023-04-29, because CountersunkClearanceHole is barely
// usable if one has to specify translation/rotation as a method argument.
module CountersunkClearanceHoleShape(diameter, height, sinkdiam=0, sinkangle=45, tolerance=0.4) {
  extra_height = 0.001 * height;
...
module CountersunkClearanceHole(diameter, height, position=[0,0,0], rotation=[0,0,0], sinkdiam=0, sinkangle=45, tolerance=0.4) {
  difference() {
    ...
    translate(position)
      rotate(rotation)
      CountersunkClearanceHoleShape(diameter, height, sinkdiam=sinkdiam, sinkangle=sinkangle, tolerance=tolerance);
  }
}
